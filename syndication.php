<!-- syndicated content on the Aspectj home page - included in aspectj.html -->

<div id="right">

<!-- ========== Eclipse portlet =============== -->

  <h2>Eclipse Links</h2>
  
  <ul>
    <li><a href="http://www.eclipse.org">Eclipse home</a></li>
	<li><a href="http://www.eclipse.org/ajdt">AJDT</a></li>
	<li><a href="http://www.eclipse.org/downloads/index.php">Eclipse downloads page</a></li>
   	<li><a href="http://www.amazon.com/exec/obidos/ASIN/0321245873/qid=1112868888/sr=2-3/ref=pd_bbs_b_2_3/103-5274059-2049410">
   	    Eclipse AspectJ</a> the book, by some of the leading AspectJ committers</li>
   </ul>	
  
   <p align="center">
    <a href="http://www.amazon.com/exec/obidos/ASIN/0321245873/qid=1112868888/sr=2-3/ref=pd_bbs_b_2_3/103-5274059-2049410">
  	   <img src="images/eclipseaspectj.jpg" width="180" alt="Eclipse AspectJ Book Cover">
    </a>
   </p>


 <!-- ============ The Aspects Blog portlet ============== -->

	<h2>The Aspects Blog</h2>
   
   <ul>
	   <?php
	
	    include './scripts/lastRSS.php';
	
	    $rss = new lastRSS;
	
	    $rss->cache_dir='.';
	    $rss->cache_time=3600;
	    $rss->date_format='l';
	    $rss->items_limit = 5;
	
	    if ($rs = $rss->get('http://www.aspectprogrammer.org/blogs/adrian/index.xml')) {
	      foreach ($rs['items'] as $item) {
	        echo "<li><a href=\"$item[link]\" title=\"$item[title]\" target=\"external\">$item[title]</a></li>\n";
	      }
	    } else {
	      echo "bad feed";
	    }
	
	   ?>
	 	<li><a href="http://www.aspectprogrammer.org/blogs/adrian"> more...</a></li>
	</ul>

<!-- ============ Aspectivity blog portlet =============== -->

	<h2>Aspectivity</h2>
	<ul>
	    <?php
	  
	      if ($rsram = $rss->get('http://ramnivas.com/blog/wp-rss2.php')) {
	        foreach ($rsram['items'] as $item) {
	          echo "<li><a href=\"$item[link]\" title=\"$item[title]\"  target=\"external\">$item[title]</a></li>\n";
	        }
	      } else {
	        echo "bad feed";
	      }
	
	    ?>
		<li><a href="http://ramnivas.com/blog/"> more...</a></li>
	</ul>
	
<!-- ========== Dion's blog portlet =============== -->

	<h2>Dion Almaer on Tech &amp; AOP</h2>
	<ul>
	  <?php
	  
	    if ($rsdion = $rss->get('http://www.almaer.com/blog/tech.xml')) {
	      foreach ($rsdion['items'] as $item) {
	        echo "<li><a href=\"$item[link]\" title=\"$item[title]\"  target=\"external\">$item[title]</a></li>\n";
	      }
	    } else {
	      echo "bad feed";
	    }
	
	  ?>
	  <li><a href="http://www.almaer.com/blog/archives/cat_aop.html"> more...</a> </li>
  </ul>
  
<!-- ============= Ron Bodkin's blog portlet ========== -->

	<h2>Ron Bodkin on AOP</h2>
	<ul>
	  <?php
	  
	    if ($rsron = $rss->get('http://rbodkin.blogs.com/ron_bodkins_blog/index.rdf')) {
	      foreach ($rsron['items'] as $item) {
	        echo "<li><a href=\"$item[link]\" title=\"$item[title]\"  target=\"external\">$item[title]</a></li>\n";
	      }
	    } else {
	      echo "bad feed";
	    }
	
	  ?>
		<li><a href="http://rbodkin.blogs.com/ron_bodkins_blog/aspectoriented_programming/index.html"> more...</a> </li>
	</ul>

<!-- ========== News portlet ================ -->
     
   <h2>AspectJ in Yahoo news...</h2>
   
   <ul>
	  <?php
	
	    if ($rs = $rss->get('http://news.search.yahoo.com/news/rss?ei=UTF-8&p=aspectj')) {
	      foreach ($rs['items'] as $item) {
	       ereg("<!\[CDATA\[(.*)]]>",$item[title], $real_title);
	       echo "<li><a href=\"$item[link]\" title=\"$real_title[1]\" target=\"external\">$real_title[1]</a></li>\n";
	     }
	    } else {
	      echo "bad feed";
	    }
	
	  ?>
	   
	   <li><a href="http://news.search.yahoo.com/search/news/?ei=ISO-8859-1&c=&p=aspectj" target=\"external\">
	      more from Yahoo...</a>
	   </li>
	   <li><a href="http://news.google.com/news?hl=en&ned=us&q=aspectj&ie=UTF-8&scoring=d" target=\"external\">
	      more from Google...</a>
	   </li>
 
   </ul>
   
</div>
